...
contract ERC1155MixedFungible is ERC1155 {
  ...
  function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) external override {
    ...
    require(_from == msg.sender || operatorApproval[_from][msg.sender] == true, "Need operator approval for 3rd party transfer.");
  ...
  function safeBatchTransferFrom(
    ...
    require(_to != address(0x0), "cannot send to zero address");
    require(_ids.length == _values.length, "length of _ids and _values much match");
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      ...
      if (isNonFungible(id)) {
        require(nfOwners[id] == _from, "NonFungible must come from owner");
        nfOwners[id] == _to;
      } else {
  ...
  function balanceOfBatch(address[] calldata _owners, uint256[] calldata _ids) external view override returns (uint256[] memory) {
    require(_owners.length == _ids.length, "Length of owners array must match length of ids");
